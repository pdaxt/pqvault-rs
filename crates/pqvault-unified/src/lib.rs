...
impl PqVaultUnified {
    ...
    #[tool(description = "Vault health summary: encryption info, key count, health status", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Get a secret value by key name. Rate-limited and usage-tracked.", annotations(read_only_hint = true))]
    async fn vault_get(
        &self,
    ...
    #[tool(description = "List all secrets with metadata and usage stats (no values shown)", annotations(read_only_hint = true))]
    async fn vault_list(
        &self,
    ...
    #[tool(description = "Search secrets by name, tag, or description", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Add a new secret to the vault with auto-detected provider and rate limits", annotations(destructive_hint = false))]
    async fn vault_add(
        &self,
    ...
    #[tool(description = "Delete a secret from the vault and remove all usage data", annotations(destructive_hint = true))]
    ...
    #[tool(description = "Import API keys from ~/.claude.json env blocks into the vault", annotations(destructive_hint = false))]
    ...
    #[tool(description = "Get .env file content for a registered project", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Write .env file for a project directly to disk -- secret values are written to file but NEVER returned to the caller.", annotations(destructive_hint = false))]
    ...
    #[tool(description = "Rotate a secret (update value and rotation timestamp)", annotations(destructive_hint = false))]
    ...
    #[tool(description = "Check rotation warnings, expired keys, orphaned keys, and smart alerts", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Full smart dashboard: all keys with usage stats, rate limits, costs, alerts", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Detailed usage stats for a specific key: requests, rate limit %, cost estimate, alerts", annotations(read_only_hint = true))]
    ...
    #[tool(description = "Proxy an API call through the vault -- key is injected as auth, never exposed to the caller. Use this instead of vault_get when you need to call an external API.", annotations(read_only_hint = true))]
